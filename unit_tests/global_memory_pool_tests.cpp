    ...
    for (boost::compute::platform& platform : platforms) {
        for (boost::compute::device& device : platform.devices()) {
            ...
            boost::compute::kernel kernel = Utils::BuildKernel(
                "VerifyHappyPathMemoryAllocSinglethreaded", context,
                Utils::CombineStrings(
                    {ProgramSourceRepository::GetGlobalMemoryPoolSource(), kSource}),
...
TEST_CASE("Happy path memory allocation from multiple threads", "[Global memory pool tests]") {
    ...
    for (boost::compute::platform& platform : platforms) {
        for (boost::compute::device& device : platform.devices()) {
            ...
            boost::compute::kernel kernel = Utils::BuildKernel(
                "VerifyHappyPathMemoryAllocMultithreaded", context,
                Utils::CombineStrings(
                    {ProgramSourceRepository::GetGlobalMemoryPoolSource(), kSource}),
                {}, {"cl_khr_global_int32_base_atomics"});
            kernel.set_arg(0, buf);
            ...
            // Kernel returned pointers stored as unsigned integers. First thing we should do is to
            // sort them so we have them in ascending order
            std::sort(results.begin(), results.end());
            // Now remove the first value from all of them (first value is a base memory address,
            // all others will be greater than it and relative to it)
            cl_ulong first_address = results.at(0);
            std::transform(
                results.begin(), results.end(), results.begin(),
                [first_address](cl_ulong val) { return val - first_address; });
...
TEST_CASE("Write data in multiple threads", "[Global memory pool tests]") {
    ...
    for (boost::compute::platform& platform : platforms) {
        for (boost::compute::device& device : platform.devices()) {
            ...
            boost::compute::kernel kernel = Utils::BuildKernel(
                "VerifyHappyPathWriteToAllocatedMemoryMultithreaded", context,
                Utils::CombineStrings(
                    {ProgramSourceRepository::GetGlobalMemoryPoolSource(), kSource}));
