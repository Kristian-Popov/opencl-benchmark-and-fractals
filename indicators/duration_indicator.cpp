        ...
        for( auto& iter_results : benchmark.durations )
        {
            for( auto& step_results : iter_results )
            {
                ...
                total_duration += step_results.second;
                {
                    ...
                    {
                        iter = calculated_.step_min_durations.emplace( step_results.first, Duration::Max() ).first;
                    }
                ...
                }
                {
                    ...
                    {
                        iter = calculated_.step_max_durations.emplace( step_results.first, Duration::Min() ).first;
                    }
                    if( step_results.second > iter->second )
                    {
                        iter->second = step_results.second;
...
bool DurationIndicator::IsEmpty() const
{
    return calculated_.total_duration == Duration();
}
