...
public:
    MappedOpenClBuffer(
        boost::compute::vector<T>& vector,
        boost::compute::command_queue& queue,
        cl_map_flags flags,
        const boost::compute::wait_list& events )
        : buffer_( vector.get_buffer() )
        , element_count_( vector.size() )
        , queue_( queue )
    {
        Map( flags, events );
    }
...
private:
    ...
    {
        host_ptr_ = static_cast<T*>(
            queue_.enqueue_map_buffer_async( buffer_, flags, 0, element_count_ * sizeof( T ), map_event_, events )
        );
    }
