    ...
    T maxRelDiff)  // TODO not sure about of these differences - T or e.g. long double?
{
    ...
    // static_assert( std::is_floating_point<T>::value, "AreFloatValuesClose function works only for
...
long double ChooseConvenientUnit(long double value, const std::vector<long double>& units) {
    ...
    auto resultIter = std::find_if(
        units.crbegin(), units.crend(), [value](long double unit) { return unit < value; });
...
long double ChooseConvenientUnit(
    ...
    std::transform(
        values.begin(), values.end(), std::back_inserter(conv_units),
        [&units](long double value) { return ChooseConvenientUnit(value, units); });
    ...
    auto resultIter =
        std::max_element(counts.begin(), counts.end(), CompareSecond<long double, int>);
...
boost::compute::program BuildProgram(
    ...
    cl_int error = 0;
    cl_program program_ = clCreateProgramWithSource(context, cl_uint(1), &source_string, 0, &error);
...
boost::compute::kernel BuildKernel(
    ...
    const std::string& buildOptions, const std::vector<std::string>& extensions) {
