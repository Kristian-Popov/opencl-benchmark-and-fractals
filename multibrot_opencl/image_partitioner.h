...
public:
    struct Segment
    {
        ...
        bool IsEmpty() const
        {
            return width_pix == 0 || height_pix == 0;
        }
    };

    ImagePartitioner(
        size_t area_width_pix, size_t area_height_pix,
        size_t fragment_width_pix, size_t fragment_height_pix )
    ...
    Segment Partition( size_t preferred_fragment_count )
    {
        ...
        result.width_pix = std::min( preferred_fragment_count * fragment_width_pix_, area_width_pix_ - result.x );
...
private:
    ...
    std::vector<Segment> segments_; // Segments are stored in a row-by-row basis in ascending order.
